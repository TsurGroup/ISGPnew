...
def create_algorithm_parameters_table():
    db_path = get_db_path()  # Pass the project_name to get_db_path
...
def save_algorithm_parameters_to_db(params: AlgorithmParameters):
    ...
    # initial_functions_text = ','.join(map(str, params.initial_functions))
    # mutation_functions_text = ','.join(map(str, params.mutation_functions))
    # Serialize upper and lower bounds with enum member names
    upper_bounds_text = json.dumps({ft.value: v for ft, v in params.upper_bounds.items()})  # Convert dict to JSON
    lower_bounds_text = json.dumps({ft.value: v for ft, v in params.lower_bounds.items()})  # Convert dict to JSON
    ...
    with conn:
        ...
        if cursor.fetchone()[0] > 0:
            ...
            ''', (
                ...
                params.mutate_probability,
                params.add_probability,
                params.stop_criteria,
                params.expected_peaks_num,
                params.norm_factor,
                params.point_diff,
                params.width_factor,
                params.alpha,
                params.population_size,
                initial_functions_text,
                mutation_functions_text,
                upper_bounds_text,
                lower_bounds_text
            ))
        else:
            ...
            cursor.execute('''
                INSERT INTO algorithm_parameters (
                    runs_num, max_generations, mutate_probability, add_probability, stop_criteria, expected_peaks_num,
                    norm_factor, point_diff, width_factor, alpha, population_size, 
...
def upsert_filter_parameters(w0, w1, use_filter):
    db_path = get_db_path()
    conn = sqlite3.connect(db_path, check_same_thread=False)

    # Use None to skip columns; SQLite will apply table defaults
    update_fields = []
    values = []
    ...
    with conn:
        cursor = conn.cursor()
        # Check if there's already a record (since there should be only one row, it's implied by "id = 1")
        cursor.execute('SELECT COUNT(*) FROM algorithm_parameters')
        if cursor.fetchone()[0] > 0:
            # Update existing record
            cursor.execute(f'''
            ...
            # Insert new record, relying on table defaults for missing values
            cursor.execute('''
                INSERT INTO algorithm_parameters (
                    w0, w1, use_filter
                ) VALUES (?, ?, ?)
            ''', (
                w0 if w0 is not None else None,
                w1 if w1 is not None else None,
                use_filter if use_filter is not None else None
            ))

    conn.commit()
    return "Row upserted successfully."
