...
class AsymmetricHyperbolicSecant(Function):
     height: float = 1
     mean: float = 0
     variance_1: float = 1
     variance_2:float = 1 
     ...
     def __init__(self, function_parameters: FunctionParameters):
        super().__init__(func_type=FunctionType.AsymmetricHyperbolicSecant, parameters_num = 4)  # Call base class constructor
     ...
     def get_value(self,x):
        return self.height / np.cosh((x - self.mean) / np.where(x < self.mean, self.variance_1, self.variance_2)) 
