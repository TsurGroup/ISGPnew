...
class NegativeGaussian(Function):
     height: float = 1
     mean: float = 0
     variance: float = 1 
     ...
     def __init__(self, function_parameters: FunctionParameters):
        super().__init__(func_type=FunctionType.NegativeGaussian, parameters_num = 3)  # Call base class constructor
     ...
     def to_string(self):
        if self.mean > 0:
           return f"{round(self.height, 5)}e^{{(\\frac{{t-{round(self.mean, 5)}}}{{{abs(round(self.variance, 5))}}})^2}}"
        if self.mean < 0:
            return f"{round(self.height, 5)}e^{{(\\frac{{t+{-round(self.mean, 5)}}}{{{abs(round(self.variance, 5))}}})^2}}"      
        return f"{round(self.height, 5)}e^{{(\\frac{{t}}{{{abs(round(self.variance, 5))}}})^2}}"
